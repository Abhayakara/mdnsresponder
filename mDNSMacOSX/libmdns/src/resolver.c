...
static void
_mdns_url_session_finalize(const mdns_url_session_t me)
{
	...
	const mdns_message_t response)
{
	...
	if (me->user_queue) {
		...
		^{
			...
			if (handler) {
				handler();
				mdns_querier_result_handler_t tmp = handler;
				BlockForget(&tmp);
			}
			mdns_release(me);
		});
...
_mdns_querier_log_query_send(const mdns_querier_t me, const mdns_session_t session)
{
	...
	os_log(_mdns_resolver_log(),
		...
		_mdns_querier_get_log_label(me),
		query_len,
		atomic_load_explicit(&me->send_count, memory_order_relaxed),
		session->server,
...
_mdns_create_empty_response_for_query(const mdns_query_message_t query, const int rcode, OSStatus * const out_error)
{
	...
	uint8_t *response_ptr = (uint8_t *)mdns_malloc(len);
...
{
	pqw_info_t *info = (pqw_info_t *)mdns_calloc(1, sizeof(*info));
...
_pqw_qname_item_create(const uint8_t * const qname, OSStatus * const out_error)
{
	...
	OSStatus err;
	pqw_qname_item_t *item = (pqw_qname_item_t *)mdns_calloc(1, sizeof(*item));
