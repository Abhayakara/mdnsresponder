	...
	xpc_object_t dns_zone_array, const char *trusted_name)
{
	...
	trusted_name_task = http_task_create_pvd_query(me->queue,
		...
		xpc_object_t tmp = candidate_dns_zones;
		xpc_forget(&tmp);
		mdns_release(service);
	});
	...
	const char *doh_uri, const char *domain)
{
	...
	uri_string = mdns_strdup(doh_uri);
	...
	const mdns_dns_service_scope_t scope, const uint32_t scoping_id)
{
	...
	for (CFIndex i = 0; i < n; ++i) {
		...
		if (scope == mdns_dns_service_scope_interface) {
			if (candidate->if_index != scoping_id) {
				continue;
			}
		} else if (scope == mdns_dns_service_scope_service) {
...
_mdns_dns_service_manager_get_interface_monitor(const mdns_dns_service_manager_t me, const uint32_t if_index)
{
	...
	^(const mdns_event_t event, __unused OSStatus error)
	{
...
#define _do_appendf(...)											\
	...
	CUClangWarningIgnoreBegin(-Wswitch-enum);
	switch (type) {
...
{
	OSStatus err;
	_domain_item_t new_item = (_domain_item_t)mdns_calloc(1, sizeof(*new_item));
	...
	new_item->name_str = mdns_strdup(normalized_name_str);
	...
	for (int32_t i = 0; i < resolver_count; ++i) {
		...
		if (new_service) {
			...
			if (new_service->if_index != 0) {
				...
				if (!name_err) {
					new_service->if_name = mdns_strdup(name_ptr);
				} else {
					os_log_error(_mdns_dns_service_log(),
	...
	OSStatus * const out_error)
{
	...
	if (interface_name) {
		service->if_name	= mdns_strdup(interface_name);
