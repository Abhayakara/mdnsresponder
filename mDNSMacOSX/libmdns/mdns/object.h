...
mdns_event_to_string(mdns_event_t event)
{
	switch (event) {
		...
		case mdns_event_update:			return "Update";
	}
	return "?";
}
...
/*!
 *	@brief
 *		Function of type mdns_context_finalizer_t that calls free() on the context argument.
 *
 *	@param context
 *		The user-defined context.
 *
 *	@discussion
 *		mdns_free_context_finalizer() is an alternative to directly using free() as a context finalizer for
 *		situations where free is considered a poisoned identifier.
 */
MDNS_SPI_AVAILABLE_FALL_2021
void
mdns_free_context_finalizer(void *context);
