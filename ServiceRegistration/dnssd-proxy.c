...
int32_t dso_transport_idle(void *context, int32_t now, int32_t next_event)
{
    return next_event;
}
...
void
dp_simple_response(comm_t *comm, message_t *message, const dns_wire_t *wire, int rcode)
{
    struct iovec iov;
    dns_wire_t response;
...
dso_send_formerr(dso_state_t *dso, const dns_wire_t *header)
{
    ...
    (void)header;
    dp_simple_response(transport, NULL, header, dns_rcode_formerr);
    return true;
}
...
dp_query_send_dns_response(dnssd_query_t *query)
{
    ...
    if (query->connection != NULL) {
        ioloop_send_message(query->connection, query->question, &iov, 1);
    }
...
dp_query_start(comm_t *comm, dnssd_query_t *query, int *rcode, DNSServiceQueryRecordReply callback)
{
    ...
    if (err != kDNSServiceErr_NoError) {
        ...
        *rcode = dns_rcode_servfail;
        ...
        query->txn = ioloop_dnssd_txn_add(sdref, query, dnssd_query_finalize_callback, dnssd_query_close_callback);
        if (query->txn == NULL) {
            return false;
        }
    ...
    if (!query->is_dns_push && local) {
        ...
        ioloop_add_wake_event(query->wakeup, query, dp_query_wakeup, NULL, ioloop_timenow() + IOLOOP_SECOND * 6);
    }
    return true;
}
                   ...
                   const char *activity_name, const char *opcode_name)
{
    ...
    if (!query) {
        dp_simple_response(comm, NULL, header, rcode);
        return;
    }
    ...
    dp_simple_response(comm, NULL, header, dns_rcode_noerror);
}
...
dns_push_reconfirm(comm_t *comm, const dns_wire_t *header, dso_state_t *dso)
{
    ...
    if (!dns_rr_parse(&question, header->data, len, &offp, false) ||
        ...
        dp_simple_response(comm, NULL, header, dns_rcode_formerr);
...
static void dso_message(comm_t *comm, const dns_wire_t *header, dso_state_t *dso)
{
    ...
    default:
        ...
        dp_simple_response(comm, NULL, header, dns_rcode_dsotypeni);
        break;
    }
...
void
dp_dns_query(comm_t *comm, message_t *message, dns_rr_t *question)
{
    int rcode;
    dnssd_query_t *query = dp_query_generate(comm, question, false, &rcode);
    const char *failnote = NULL;
    if (!query) {
        dp_simple_response(comm, message, &message->wire, rcode);
        return;
    }
...
void dns_evaluate(comm_t *comm, message_t *message)
{
    dns_rr_t question;
    unsigned offset = 0;
    ...
    case dns_opcode_dso:
        if (!comm->tcp_stream) {
            ...
            dp_simple_response(comm, message, &message->wire, dns_rcode_notimp);
            return;
        }
        ...
        if (!comm->dso) {
            ...
            if (!comm->dso) {
                ...
                dp_simple_response(comm, message, &message->wire, dns_rcode_servfail);
                ioloop_close(&comm->io);
                return;
            }
            comm->dso->transport_finalize = dso_transport_finalize;
        }
        dso_message_received(comm->dso, (uint8_t *)&message->wire, message->length);
        break;
    ...
    case dns_opcode_query:
        ...
        if (ntohs(message->wire.qdcount) != 1) {
            dp_simple_response(comm, message, &message->wire, dns_rcode_formerr);
            return;
        }
        ...
        dp_dns_query(comm, message, &question);
...
void dns_input(comm_t *comm, message_t *message, void *context)
{
...
{
    served_domain_t *sd;
    int ifindex = if_nametoindex(name); // XXX
...
dnssd_push_setup()
{
    ...
    listener[num_listeners] = ioloop_listener_create(true, true, NULL, 0, &addr, NULL,
                                                    "DNS Push Listener", dns_input, NULL, NULL, NULL, NULL, NULL);
...
main(int argc, char **argv)
{
    ...
    addr.sa.sa_len = sizeof(addr.sin6);
#endif
    addr.sin6.sin6_port = htons(udp_port);
    for (i = 0; i < num_listen_addrs; i++) {
        if (!getipaddr(&addr, listen_addrs[i])) {
            ERROR("UDP listener %s: Invalid IP address", listen_addrs[i]);
            return 1;
        }
    ...
    addr.sa.sa_len = sizeof(addr.sin6);
#endif
    addr.sin6.sin6_port = htons(tcp_port);
    listener[num_listeners] = ioloop_listener_create(true, false, NULL, 0, &addr, NULL,
                                                     "TCP DNS Listener", dns_input, NULL, NULL, NULL, NULL, NULL);
    ...
    // If we haven't been given any addresses to listen on, listen on the unspecified address for IPv4 and IPv6
