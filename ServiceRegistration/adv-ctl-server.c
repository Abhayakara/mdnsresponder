...
adv_ctl_message_parse(advertising_proxy_conn_ref connection)
{
    ...
    case kDNSSDAdvertisingProxyEnable:
        INFO("adv_ctl_message_parse: Client uid %d pid %d sent a kDNSSDAdvertisingProxyEnable request.",
			 connection->uid, connection->gid);
		break;
	case kDNSSDAdvertisingProxyListServiceTypes:
        INFO("adv_ctl_message_parse: Client uid %d pid %d sent a kDNSSDAdvertisingProxyListServiceTypes request.",
			 connection->uid, connection->gid);
		break;
	case kDNSSDAdvertisingProxyListServices:
        INFO("adv_ctl_message_parse: Client uid %d pid %d sent a kDNSSDAdvertisingProxyListServices request.",
			 connection->uid, connection->gid);
        adv_ctl_list_services(connection);
		return;
	case kDNSSDAdvertisingProxyListHosts:
        INFO("adv_ctl_message_parse: Client uid %d pid %d sent a kDNSSDAdvertisingProxyListHosts request.",
			 connection->uid, connection->gid);
		break;
	case kDNSSDAdvertisingProxyGetHost:
        INFO("adv_ctl_message_parse: Client uid %d pid %d sent a kDNSSDAdvertisingProxyGetHost request.",
			 connection->uid, connection->gid);
		break;
	case kDNSSDAdvertisingProxyFlushEntries:
        INFO("adv_ctl_message_parse: Client uid %d pid %d sent a kDNSSDAdvertisingProxyFlushEntries request.",
			 connection->uid, connection->gid);
        srp_mdns_flush();
		break;
	case kDNSSDAdvertisingProxyBlockService:
        INFO("adv_ctl_message_parse: Client uid %d pid %d sent a kDNSSDAdvertisingProxyBlockService request.",
			 connection->uid, connection->gid);
        adv_ctl_block_service(true);
		break;
	case kDNSSDAdvertisingProxyUnblockService:
        INFO("adv_ctl_message_parse: Client uid %d pid %d sent a kDNSSDAdvertisingProxyUnblockService request.",
			 connection->uid, connection->gid);
        adv_ctl_block_service(false);
		break;
	case kDNSSDAdvertisingProxyRegenerateULA:
        INFO("adv_ctl_message_parse: Client uid %d pid %d sent a kDNSSDAdvertisingProxyRegenerateULA request.",
			 connection->uid, connection->gid);
