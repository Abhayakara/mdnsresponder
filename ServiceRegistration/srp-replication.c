...
static void
srpl_connection_attachments_free(srpl_connection_t *srpl_connection)
{
...
srpl_connection_finalize(srpl_connection_t *srpl_connection)
{
    ...
    srpl_connection_attachments_free(srpl_connection);
...
srpl_host_message_send(srpl_connection_t *srpl_connection, adv_host_t *host)
{
    ...
    INFO(PRI_S_SRP " sent SRPLHost message %02x%02x " PRI_S_SRP " on connection.",
         srpl_connection->name, message.buf[0], message.buf[1], host->name);
    return true;
}
...
srpl_host_message(srpl_connection_t *srpl_connection, message_t *message, dso_state_t *dso)
{
    if (dso->primary.length != 0) {
        ...
        dns_name_print(event.content.host_update.hostname, hostname, sizeof(hostname));

        INFO(PRI_S_SRP " received SRPLHost message %x for host " PRI_S_SRP,
             srpl_connection->name, ntohs(message->wire.id), hostname);
    ...
    return;
fail:
    INFO(PRI_S_SRP " received bogus SRPLHost message %x", srpl_connection->name, ntohs(message->wire.id));
    srpl_disconnect(srpl_connection);
    return;}
...
srpl_candidate_host_contention_wait_action(srpl_connection_t *srpl_connection, srpl_event_t *event)
{
    ...
    if (event == NULL) {
        ...
        adv_host_t *host = event->content.advertise_finished.host;
        if (dns_names_equal_text(srpl_connection->candidate->name, host->name)) {
            return srpl_state_candidate_host_re_evaluate;
        }
        return srpl_state_invalid;
    } else {
...
static void
srpl_deferred_advertise_finish_event_deliver(void *context)
{
    srpl_event_t *event = context;
    srpl_event_deliver(event->srpl_connection, event);
    RELEASE_HERE(event->srpl_connection, srpl_connection_finalize);
    srp_adv_host_release(event->content.advertise_finished.host);
    free(event);
}

static void
srpl_advertise_finished_event_defer(srpl_connection_t *srpl_connection, srpl_event_t *event_to_defer)
{
    srpl_event_t *event = calloc(1, sizeof(*event));
    if (event == NULL) {
        ERROR("No memory to defer advertise_finished event " PUB_S_SRP " on connection " PRI_S_SRP,
              event_to_defer->name, srpl_connection->name);
        return;
    }

    srpl_event_initialize(event, srpl_event_advertise_finished);
    event->content.advertise_finished.rcode = event_to_defer->content.advertise_finished.rcode;
    event->content.advertise_finished.host = event_to_defer->content.advertise_finished.host;
    srp_adv_host_retain(event->content.advertise_finished.host);
    event->srpl_connection = srpl_connection;
    RETAIN_HERE(event->srpl_connection);
    ioloop_run_async(srpl_deferred_advertise_finish_event_deliver, event);
}
...
srpl_candidate_host_apply_action(srpl_connection_t *srpl_connection, srpl_event_t *event)
{
    ...
    if (event == NULL) {
        ...
        // We can get an advertise_finished event while still in candidate_host_apply if the update didn't change anything.
        srpl_advertise_finished_event_defer(srpl_connection, event);
        return srpl_state_invalid;
    } else {
        UNEXPECTED_EVENT(srpl_connection, event);
    }
...
void
srpl_advertise_finished_event_send(adv_host_t *host, int rcode)
{
    srpl_event_t event;

    srpl_event_initialize(&event, srpl_event_advertise_finished);
    event.content.advertise_finished.rcode = rcode;
    event.content.advertise_finished.host = host;
...
srpl_ready_action(srpl_connection_t *srpl_connection, srpl_event_t *event)
{
    ...
    if (event == NULL) {
        ...
        if (srpl_connection->stashed_host.hostname != NULL &&
            dns_names_equal_text(srpl_connection->stashed_host.hostname, event->content.advertise_finished.host->name))
        {
...
srpl_srp_client_update_send_action(srpl_connection_t *srpl_connection, srpl_event_t *event)
{
    ...
    if (update != NULL) {
        // A removed host won't be added, but it's possible that its lease could expire before we get a chance to process it
        // in which case we shouldn't send a host message.
        if (srp_adv_host_valid(update->host) && update->host->message != NULL) {
            ...
            if (update->host->message == NULL) {
                ...
                if (update->host->removed) {
                    ERROR(PRI_S_SRP ": host " PRI_S_SRP " is removed.", srpl_connection->name, update->host->name);
                } else  {
                    ERROR(PRI_S_SRP ": host " PRI_S_SRP " has no key.", srpl_connection->name, update->host->name);
                }
            }
...
srpl_stashed_host_apply_action(srpl_connection_t *srpl_connection, srpl_event_t *event)
{
    ...
    if (event == NULL) {
        ...
        // We can get an advertise_finished event while still in stashed_host_apply if the update didn't change anything.
        srpl_advertise_finished_event_defer(srpl_connection, event);
        return srpl_state_invalid;
    } else {
        UNEXPECTED_EVENT(srpl_connection, event);
...
static void
srpl_srp_client_update_send_event_to_connection(srpl_connection_t *srpl_connection, adv_host_t *host, int rcode)
{
    srpl_event_t event;
...
srpl_srp_client_update_finished_event_send(adv_host_t *host, int rcode)
{
    for (srpl_domain_t *domain = srpl_domains; domain != NULL; domain = domain->next) {
        for (srpl_instance_t *instance = domain->instances; instance != NULL; instance = instance->next) {
            if (instance->outgoing != NULL) {
                srpl_srp_client_update_send_event_to_connection(instance->outgoing, host, rcode);
            }
            if (instance->incoming != NULL) {
                srpl_srp_client_update_send_event_to_connection(instance->incoming, host, rcode);
            }
        }
    }
}
