...
static bool
srp_replication_advertise_finished(adv_host_t *host, void *context, comm_t *connection, int rcode)
{
	if (srp_replication_enabled) {
        INFO("replication advertise finished: host = %p  context = %p  connection = %p  rcode = " PUB_S_SRP,
             host, context, connection, dns_rcode_name(rcode));
        if (connection == NULL) {
            // This is an SRP replication update, not an actual client we're communicating with.
            // In this case, host should never be NULL.
            if (context != NULL) {
                srpl_advertise_finished_event_send(host, rcode);
...
static void
advertise_finished(adv_host_t *host, void *context, comm_t *connection, message_t *message, int rcode, client_update_t *client)
{
    struct iovec iov;
    dns_wire_t response;

#if SRP_FEATURE_REPLICATION
    if (srp_replication_advertise_finished(host, context, connection, rcode)) {
        return;
    }
#else
    (void)host;
...
update_failed(adv_update_t *update, int rcode, bool expire)
{
    ...
    if (update->client != NULL) {
        ...
        advertise_finished(host, host->srpl_connection, client->connection, client->message, rcode, NULL);
...
update_finished(adv_update_t *update, bool not_really)
{
    ...
    if (client) {
        ...
        advertise_finished(host, host->srpl_connection, client->connection, client->message, dns_rcode_noerror, client);
        client_finalize(client);
        update->client = NULL;
                             ...
                             const char *name, const char *regtype, const char *domain, void *context)
{
    ...
    if (update != NULL && host->updates != update) {
        INFO("registration for service " PRI_S_SRP "." PRI_S_SRP
...
start_service_updates(adv_host_t *host)
{
    ...
    if (update->num_instances_started == 0) {
        INFO("no service updates, so we're finished.");
        update_finished(update, false);
    }
}
                         ...
                         DNSServiceFlags flags, DNSServiceErrorType error_code, void *context)
{
    ...
    if (update != NULL && (host == NULL || host->updates != update)) {
        INFO("registration for host completed with invalid state.");
        update_finalize(update);
        return;
    }
    ...
    if (error_code == kDNSServiceErr_NoError) {
        ...
        INFO("registration for host " PRI_S_SRP " has completed.", host->registered_name);
        start_service_updates(host);
    } else {
        INFO("registration for host " PRI_S_SRP " failed, status = %d", host->registered_name,
             error_code);
        if (update == NULL) {
...
start_host_update(adv_host_t *host)
{
    ...
    if (selected_addr == NULL) {
        ...
        if (remove_preexisting || host->conn == NULL) {
            if (host->num_addresses > 0) {
                INFO("Replacing existing address %p.", selected_addr);
            } else {
                INFO("Adding new address %p.", selected_addr);
            }
            add_rdata = selected_addr->rdata;
            add_rrtype = selected_addr->rrtype;
            add_rdlen = selected_addr->rdlen;
        } else {
            INFO("Retaining existing address.");
        }
        update->registering_key = false;
        if (update->selected_addr != NULL) {
            RELEASE_HERE(update->selected_addr, adv_address_finalize);
    ...
    if (add_rdata != NULL) {
        ...
        INFO("DNSServiceRegisterRecord(%p %p %d %d %p %d %d %d %p %d %p %p)",
    ...
    // If we didn't have to do an add, start the service updates immediately.
        ...
        INFO("no host address rdata, so no host update");
        start_service_updates(host);
    }
    return;
}
...
found_something:
    if (outcome == conflict) {
        ...
        advertise_finished(NULL, context, connection, raw_message, dns_rcode_yxdomain, NULL);
        goto cleanup;
    }
    ...
    if (outcome == missed) {
        if (remove) {
            ...
            advertise_finished(NULL, context, connection, raw_message, dns_rcode_nxdomain, NULL);
            goto cleanup;
        }
        ...
        if (host == NULL) {
            ...
            advertise_finished(NULL, context, connection, raw_message, dns_rcode_servfail, NULL);
            goto cleanup;
        }
        host->ref_count = 1;
...
#if SRP_FEATURE_REPLICATION
            srp_replication_advertise_finished(host, context, connection, dns_rcode_servfail);
#endif
        cleanup:
    ...
    if (remove) {
        host_invalidate(host);
        advertise_finished(NULL, context, connection, raw_message, dns_rcode_noerror, NULL);
        goto cleanup;
    }
    ...
    *p_client_update = client_update;
    ...
    if (host->updates == NULL) {
        INFO("No ongoing host update: preparing this update to be applied.");
        prepare_update(host);
    } else {
        INFO("waiting for existing update to complete on host " PRI_S_SRP " before applying another.",
             host->name);
    }
    return true;
}
